...
from input_data import preprocess_data,load_sz_data,load_los_data,load_st_data
...
flags.DEFINE_integer('seq_len', 12 , '  time le ngth of inputs.')
flags.DEFINE_integer('pre_len', 12, 'time length of prediction.')
flags.DEFINE_float('train_rate', 0.67, 'rate of training set.')
flags.DEFINE_integer('batch_size', 32, 'batch size.')
flags.DEFINE_string('dataset', 'st', ' los or st.')
flags.DEFINE_string('model_name', 'tgcn', 'gcngru')
...
def TGCN(_X, _weights, _biases):
    ...
    cell = tf.nn.rnn_cell.MultiRNNCell([cell_1], state_is_tuple=True)#实现多层神经网络
    _X = tf.unstack(_X, axis=1)#降1维
...
# var.to_csv(path+'/test_truth.csv',index = False,header = False)
